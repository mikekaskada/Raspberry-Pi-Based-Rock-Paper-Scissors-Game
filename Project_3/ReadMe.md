# Αναγώριση χειρονομιών μέσω της κάμερας του Raspberry Pi και χρήση ΤΝ για την στρατηγική με την οποία παίζει ο υπολογιστής
Προκειται για τον συνδυασμό των δύο προηγούμενων έργων, δηλαδή της αναγνώρισης των χειρονοομιών που κάνει ο παίκτης και το παίξιμο του υπολογιστή με την χρήση ΤΝ με σκοπό να βρεί μοτίβα στο παίξιμο των ανθρώπων.

Τοποθετούμε την κάμερα πάνω σε στοίβα βιβλίων και κουτιών περίπου 1 m πάνω από την επιφάνεια ενός τραπεζιού ή γραφείου. 
...
Με το μονόχρωμο (άσπρο) υπόβαθρο είναι πιο εύκολη η αναγνώριση και κατηγοριοποίηση της χειρονομίας από το πρόγραμμα.
Χρησιμοποιούμε τον κώδικα `Rock_Paper_Scissors_with_AI_Agent_and_Gesture_Recognition.py`
...
1. **Epsilon-Greedy Strategy:**
   ...
   - **Παραμετροποίηση:** Ρυθμίστε την αρχική τιμή του epsilon μεταξύ `0.1` και `1.0`. Ρυθμίστε τον ρυθμό μείωσης (`epsilon_decay`) μεταξύ `0.9` και `0.999`. Για παράδειγμα, δοκιμάστε `epsilon = 0.7` και `epsilon_decay = 0.99` για ισορροπία μεταξύ εξερεύνησης και εκμετάλλευσης.
...
2. **Q-Network and Target Network:**
   ...
   - **Παραμετροποίηση:** Ρυθμίστε τη συχνότητα ενημέρωσης του target network κάθε `1000` έως `5000` βήματα. Δοκιμάστε να ενημερώνετε το target network κάθε `3000` βήματα για σταθερή εκπαίδευση.

3. **Experience Replay:**
   - Αποθηκεύει εμπειρίες και δειγματοληπτεί παρτίδες για την εκπαίδευση του Q-network.
   - **Batch Size:** Συνήθως `64`.
   - **Παραμετροποίηση:** Ρυθμίστε το μέγεθος παρτίδας μεταξύ `32` και `128` για να δείτε πώς επηρεάζει τη σταθερότητα και την απόδοση της εκπαίδευσης. Δοκιμάστε παρτίδες των `64` και `96` για να βρείτε την καλύτερη ισορροπία.
...
4. **Pattern Detection with LSTM:**
   ...
   - **Παραμετροποίηση:** Δοκιμάστε διαφορετικές διαστάσεις κρυφών στρώσεων (π.χ. `50`, `100`) και αριθμούς στρώσεων (π.χ. `1`, `3`) στο LSTM για να βρείτε τη βέλτιστη ρύθμιση για την ανίχνευση προτύπων.
...
5. **Adaptive Strategy:**
   ...
   - **Παραμετροποίηση:** Προσαρμόστε τα κατώφλια για την ταξινόμηση της συμπεριφοράς του αντιπάλου, π.χ. επιθετική αν μία κίνηση εμφανίζεται πάνω από `60%`, αμυντική αν καμία δεν ξεπερνάει το `40%`, και τυχαία για ενδιάμεσες τιμές. Δοκιμάστε διαφορετικά ποσοστά όπως `50%` και `30%`.
...
6. **Dynamic Learning Rate Adjustment:**
   ...
   - **Παραμετροποίηση:** Ρυθμίστε τα ποσοστά αύξησης και μείωσης του ρυθμού μάθησης κατά `5%` έως `20%`. Δοκιμάστε να αυξάνετε τον ρυθμό μάθησης κατά `10%` και να τον μειώνετε κατά `10%` για να δείτε πώς επηρεάζει την απόδοση.
...
7. **Real-Time Learning:**
   ...
   - **Παραμετροποίηση:** Δοκιμάστε διαφορετικά ποσοστά ενημέρωσης, π.χ. κάθε `1`, `5`, ή `10` κινήσεις. Εξετάστε την προσαρμογή του epsilon μετά από κάθε `1` κίνηση και τον ρυθμό μάθησης κάθε `5` κινήσεις για να δείτε πώς επηρεάζει την απόδοση του πράκτορα.
...
8. **Pre-training from Historical Data:**
   ...
   - **Παραμετροποίηση:** Δοκιμάστε διαφορετικά μεγέθη και είδη ιστορικών δεδομένων, π.χ. `1000`, `5000`, ή `10000` κινήσεις, για την προεκπαίδευση για να δείτε πώς επηρεάζουν την αρχική απόδοση του πράκτορα.
...
9. **Cluster Analysis:**
   ...
   - **Παραμετροποίηση:** Δοκιμάστε διαφορετικούς αριθμούς clusters (`3`, `5`, `7`, `10`) και παραμέτρους KMeans για να βρείτε την καλύτερη ομαδοποίηση στρατηγικών αντιπάλων. Πειραματιστείτε με `n_clusters = 5` και `n_clusters = 7` για διαφορετικές ομαδοποιήσεις.
