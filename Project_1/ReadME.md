...
Αν και η εγκυρότητα σε ένα μοντέλο μηχανικής μάθησης είναι κάτι διαφορετικό, επειδή δεν μπορούμε να την εφαρμόσουμε στο Teachable Machine, δημιουργήσαμε μια άλλη διαδικασία για τον έλεγχο του μοντέλου.
1. Τρέχουμε τον κώδικα `random move validation data.py`. Ο κώδικας μετακινεί και μετονομάζει 5000 τυχαία επιλεγμένες εικόνες από κάθε κατηγορία, που δεν χρησιμοποιήθηκαν στην εκπαίδευση του μοντέλου στον φάκελο `validation_data`. Έτσι έχουμε 15000 εικόνες **paper**, **scissors** , **rock** μέσα στον ίδιο φάκελο, τον φάκελο `validation_data`.
2. Τρέχουμε τον κώδικα `Validate data.py`. Ο κώδικας χρησιμοποιεί το μοντέλο που εκπαιδεύσαμε και δημιουργήσαμε με το Teachable Machine και προσπαθεί να αναγνωρίσει τις εικόνες που είναι στον φάκελο `validation_data` και τις τοποθετεί σε υποφακέλους `paper`, `scissors`, `rock` σύνφωνα με την αναγνώριση που έκανε. ***'Οσες εικόνες δεν αναγνωριστούν σωστά θα είναι τοποθετημένες σε λάθος φάκελο. Ένας οπτικός έλεγχος στον κάθε φάκελο μας δείχνει πόσο ακριβές είναι το μοντέλο.
Στην επόμενη εικόνα χειρονομίες "ψαλίδι" αναγνωρίστηκαν ως "χαρτί" και μπήκαν στον φάκελο `paper`. 

![15](https://github.com/mikekaskada/Raspberry-Pi-Based-Rock-Paper-Scissors-Game/blob/main/Project_1/Images/Screenshot_9.jpg)

**H διαδικασία αναγνώρισης από το μοντέλο χρειάζόνταν περίπου 35 ms (35/1000 sec- χιλιοστά του δευτερολέπτου) ανά εικόνα στο PC, ενώ όταν το δοκιμάσαμε στο Raspberry Pi περίπου 105 ms.**
